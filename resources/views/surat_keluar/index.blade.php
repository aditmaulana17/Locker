...
@php
    ...
    try{
        if($dariTanggal&&$sampaiTanggal){
            $visibleDateRange=\Illuminate\Support\Carbon::parse($dariTanggal)->format('d/m/Y').' - '.\Illuminate\Support\Carbon::parse($sampaiTanggal)->format('d/m/Y');
        }elseif($dariTanggal){
    ...
    $hasFilters=request()->filled('search')||!empty($selectedKategori)||!empty($selectedStatus)||request()->filled('dari_tanggal')||request()->filled('sampai_tanggal');
@endphp
...
.custom-date-picker-header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:13px 15px;border-bottom:1px solid #cbd5e1}
...
.custom-calendar{padding:15px 17px 13px}
.custom-calendar+.custom-calendar{border-left:1px solid #cbd5e1}
.custom-calendar-head{display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:7px}
...
.custom-calendar-month-button,.custom-calendar-year-button{display:inline-flex;align-items:center;justify-content:center;gap:6px;min-height:33px;padding:6px 10px;border:1px solid transparent;border-radius:9px;background:#f8fafc;color:#334155;font-family:inherit;font-size:12px;font-weight:800;cursor:pointer}
.custom-calendar-month-button:hover,.custom-calendar-year-button:hover{border-color:#bfdbfe;background:#eff6ff;color:#2563eb}
...
.custom-calendar-weekdays,.custom-calendar-days{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px}
...
.custom-date-picker-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 14px;border-top:1px solid #cbd5e1}
.custom-date-picker-selected{min-width:0;color:#64748b;font-size:11px;font-weight:700}
.custom-date-picker-actions{display:flex;align-items:center;gap:7px}
.custom-date-picker-button{height:36px;padding:0 14px;border:1px solid #cbd5e1;border-radius:9px;background:#f8fafc;color:#475569;font-family:inherit;font-size:11px;font-weight:700;cursor:pointer}
...
.custom-picker-panel{position:fixed;z-index:1000000;width:320px;max-width:calc(100vw - 20px);padding:14px;border-radius:16px}
.custom-picker-panel-header{display:flex;align-items:center;justify-content:space-between;gap:7px;margin-bottom:12px}
.custom-picker-panel-title{flex:1;color:#334155;font-size:13px;font-weight:800;text-align:center}
.custom-picker-panel-nav,.custom-picker-panel-close{width:32px;height:32px;flex:0 0 32px;border-radius:9px;font-size:19px}
.custom-picker-panel-nav{border:0;background:#f8fafc;color:#64748b;cursor:pointer}
.custom-picker-panel-nav:hover{background:#eff6ff;color:#2563eb}
.custom-picker-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}
.custom-picker-option{min-height:42px;padding:0 6px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;color:#475569;font-family:inherit;font-size:11px;font-weight:700;cursor:pointer;transition:background-color .15s ease,border-color .15s ease,color .15s ease}
...
.filter-row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
.filter-dropdown{position:relative;min-width:0}
.filter-dropdown-trigger{display:flex;width:100%;min-height:56px;align-items:center;gap:12px;padding:10px 14px;border:1px solid #cbd5e1;border-radius:14px;background:#fff;color:#334155;text-align:left;cursor:pointer;transition:border-color .15s ease,background-color .15s ease,box-shadow .15s ease}
.filter-dropdown-trigger:hover{border-color:#94a3b8;background:#f8fafc}
.filter-dropdown-trigger[aria-expanded="true"]{border-color:#93c5fd;background:#f8fbff;box-shadow:0 0 0 3px rgba(37,99,235,.08)}
.filter-dropdown-trigger.status-trigger[aria-expanded="true"]{border-color:#fde68a;background:#fffcf0;box-shadow:0 0 0 3px rgba(245,158,11,.08)}
.filter-dropdown-trigger-content{display:flex;min-width:0;flex:1;align-items:center;gap:10px}
.filter-dropdown-icon{display:inline-flex;width:36px;height:36px;align-items:center;justify-content:center;flex:0 0 36px;border-radius:10px;background:#eff6ff;color:#2563eb}
...
.filter-dropdown-title{color:#334155;font-size:13px;font-weight:700}
.filter-dropdown-subtitle{margin-top:3px;color:#94a3b8;font-size:10px;font-weight:500}
.filter-dropdown-count{display:inline-flex;min-width:72px;min-height:25px;align-items:center;justify-content:center;flex:0 0 auto;border-radius:999px;padding:0 9px;font-size:10px;font-weight:700;white-space:nowrap}
...
.filter-dropdown-arrow{flex:0 0 auto;color:#94a3b8;transition:transform .15s ease,color .15s ease}
...
.filter-dropdown-menu{position:absolute;top:calc(100% + 8px);right:0;left:0;z-index:10020;display:none;overflow:hidden;border:1px solid #94a3b8;border-radius:14px;background:#fff;box-shadow:0 20px 50px rgba(15,23,42,.14),0 6px 18px rgba(15,23,42,.07)}
.filter-dropdown.open .filter-dropdown-menu{display:block}
.filter-dropdown-menu-header{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid #cbd5e1;background:#f8fafc}
.filter-dropdown-menu-title-wrap{min-width:0}
.filter-dropdown-menu-title{display:block;color:#334155;font-size:12px;font-weight:800}
.filter-dropdown-menu-description{display:block;margin-top:3px;color:#94a3b8;font-size:10px}
.filter-dropdown-actions{display:inline-flex;align-items:center;gap:5px;flex:0 0 auto}
.filter-dropdown-action{border:0;border-radius:7px;background:transparent;padding:6px 8px;font-size:10px;font-weight:700;cursor:pointer}
...
.filter-dropdown-options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;max-height:280px;overflow-y:auto;padding:12px}
.filter-dropdown-options::-webkit-scrollbar{width:6px}
.filter-dropdown-options::-webkit-scrollbar-track{background:#f8fafc}
.filter-dropdown-options::-webkit-scrollbar-thumb{border-radius:999px;background:#cbd5e1}
.filter-dropdown-option{display:flex;min-width:0;min-height:44px;align-items:center;gap:9px;padding:8px 10px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;cursor:pointer;transition:border-color .15s ease,background-color .15s ease}
.filter-dropdown-option:hover{border-color:#93c5fd;background:#f8fbff}
.filter-dropdown-option.status-option:hover{border-color:#fde68a;background:#fffcf3}
.filter-dropdown-option:has(input:checked){border-color:#93c5fd;background:#eff6ff}
.filter-dropdown-option.status-option:has(input:checked){border-color:#fde68a;background:#fffbeb}
.filter-dropdown-option input{width:16px;height:16px;margin:0;accent-color:#2563eb;cursor:pointer}
.filter-dropdown-option span{min-width:0;overflow:hidden;color:#475569;font-size:11px;font-weight:600;line-height:1.3;text-overflow:ellipsis;white-space:nowrap}
...
.filter-dropdown-empty{padding:22px 12px;color:#94a3b8;font-size:11px;text-align:center}
.filter-dropdown-menu-footer{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 14px;border-top:1px solid #cbd5e1}
.filter-dropdown-footer-count{color:#64748b;font-size:10px;font-weight:600}
.filter-dropdown-footer-hint{color:#cbd5e1;font-size:10px}
...
.archive-table .cell-date{color:#475569;font-weight:600}
...
.archive-table .action-button{display:inline-flex;width:30px;height:30px;align-items:center;justify-content:center;border-radius:8px;transition:background .15s ease,color .15s ease}
...
@media(max-width:767px){
    ...
    .custom-calendar-day{height:39px}
    ...
    .filter-dropdown-options{max-height:calc(80vh - 155px);grid-template-columns:1fr 1fr}
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto">
            <a href="{{ route('export.surat-keluar.excel',request()->query()) }}" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs font-semibold text-emerald-700 shadow-sm transition hover:bg-emerald-100 sm:px-3.5">
            ...
            <a href="{{ route('export.surat-keluar.pdf',request()->query()) }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:bg-rose-100 sm:px-3.5">
    ...
    <div class="rounded-2xl border border-slate-300 bg-white p-3 shadow-sm sm:p-4">
        <form id="filterForm" method="GET" action="{{ route('surat-keluar.index') }}" class="space-y-3">
            ...
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                ...
                <div class="lg:col-span-5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="search" name="search" value="{{ request('search') }}" placeholder="Cari perihal, nomor surat, atau instansi..." autocomplete="off" class="h-11 w-full rounded-xl border border-slate-300 bg-slate-50 pl-9 pr-3 text-xs text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 sm:text-sm">
                    </div>
                </div>
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        @if($hasFilters)
                            <a href="{{ route('surat-keluar.index') }}" title="Reset Filter" aria-label="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                            </a>
            ...
            <div class="filter-row">
                ...
                <div class="filter-dropdown" id="kategoriDropdown">
                    ...
                    <button type="button" id="kategoriDropdownButton" class="filter-dropdown-trigger" aria-expanded="false" aria-controls="kategoriDropdownMenu">
                        ...
                        <svg class="filter-dropdown-arrow h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @forelse($suratKeluars??[] as $s)
                        ...
                        <tr>
                            ...
                            <td class="cell-sender">
                                <span class="sender-badge" title="{{ $tujuan }}">
                                    {{ $tujuan }}
                                </span>
                            </td>

                            <td class="cell-subject max-w-xs truncate" title="{{ $s->perihal??'-' }}">
                                {{ $s->perihal??'-' }}
                            </td>
...
<div id="customDatePicker" class="custom-date-picker hidden">
    ...
    <div class="custom-date-picker-header">
        ...
        <button type="button" id="datePickerClose" class="custom-date-picker-close" aria-label="Tutup">
            &times;
        </button>
    </div>
...
(function(){
    ...
    let viewMonth=selectedStart?new Date(selectedStart.getFullYear(),selectedStart.getMonth(),1):new Date();
    ...
    function renderCalendars(){
        ...
        const leftDate=new Date(viewMonth.getFullYear(),viewMonth.getMonth(),1);
    ...
    function positionDatePicker(){
        if(!datePicker||!dateInput||window.innerWidth<=767)return;

        const rect=dateInput.getBoundingClientRect();
        const width=datePicker.offsetWidth||720;
        const height=datePicker.offsetHeight||500;
    ...
    function showMonthPanel(calendarDate,anchor,side){
        ...
        header.append(previous,title,next,close);
        ...
        function renderMonths(){
            title.textContent=String(activePanelYear);
            grid.innerHTML='';
            ...
            positionPickerPanel(pickerPanel,anchor);
        }
    ...
    function clearDateRange(){
        ...
        if(clearDateButton){
            clearDateButton.classList.remove('flex');
    ...
    if(dateInput&&datePicker){
        ...
        closeDatePickerButton?.addEventListener('click',function(event){
            event.preventDefault();
            event.stopPropagation();
            hideDatePicker();
        });
        ...
        clearDateButton?.addEventListener('click',function(event){
            event.preventDefault();
            event.stopPropagation();
            clearDateRange();
        });
        ...
        document.addEventListener('mousedown',function(event){
            ...
            if(pickerPanel&&pickerPanel.contains(event.target))return;
            if(dateInput.contains(event.target))return;
            if(clearDateButton&&clearDateButton.contains(event.target))return;
    ...
    function updateKategoriFilter(){
        ...
        if(kategoriSummary){
            ...
            if(!labels.length){
                ...
                kategoriSummary.textContent=labels.length+' kategori dipilih';
            }
        }
    }
    ...
    function updateStatusFilter(){
        ...
        if(statusSummary){
            ...
            if(!labels.length){
                ...
                statusSummary.textContent=labels.length+' status dipilih';
            }
        }
    }
    ...
    document.getElementById('selectAllKategori')?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
    ...
    document.getElementById('clearAllKategori')?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
    ...
    document.getElementById('selectAllStatus')?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
    ...
    document.getElementById('clearAllStatus')?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
