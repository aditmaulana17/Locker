...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto">
            ...
            <a href="{{ route('export.surat-keluar.pdf', request()->query()) }}" target="_blank" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:bg-rose-100 sm:px-3.5">
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm sm:p-5">
        <form method="GET" action="{{ route('surat-keluar.index') }}" id="filterForm" class="space-y-3">
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        @if($hasFilters)
                            <a href="{{ route('surat-keluar.index') }}" title="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </a>
                        @endif
                    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    @forelse($suratKeluars ?? [] as $s)
                        ...
                        <tr class="transition duration-150 hover:bg-slate-50/60">
                            <td class="px-4 py-3.5 text-slate-500 sm:px-6 sm:py-4">{{ optional($s->tanggal_surat)->format('d/m/Y') ?? '-' }}</td>
                            <td class="px-4 py-3.5 text-slate-700 sm:px-6 sm:py-4">
                                <span class="inline-block max-w-[220px] truncate rounded-lg border border-slate-200/60 bg-slate-100 px-2.5 py-1 font-medium text-slate-700" title="{{ $s->instansi_tujuan ?? '-' }}">{{ $s->instansi_tujuan ?? '-' }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-center sm:px-6 sm:py-4">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('surat-keluar.show', $s) }}" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-blue-50 hover:text-blue-600" title="Lihat Detail">
                                    ...
                                    @if(in_array($userRole, ['admin', 'pimpinan']))
                                        <a href="{{ route('surat-keluar.edit', $s) }}" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-amber-50 hover:text-amber-600" title="Ubah Data">
        ...
        @if(isset($suratKeluars) && method_exists($suratKeluars, 'hasPages') && $suratKeluars->hasPages())
            <div class="border-t border-slate-100 px-4 py-3 sm:px-6 sm:py-4">{{ $suratKeluars->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
