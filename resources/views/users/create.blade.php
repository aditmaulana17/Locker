    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-sm border border-slate-200/80">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Tambah Pengguna Baru</h1>
            </div>
            <p class="text-xs sm:text-sm text-slate-500 mt-1">Buat akun kredensial baru dan tentukan hak akses keamanan sistem.</p>
        </div>
            ...
            <div class="p-4 sm:p-8 space-y-5 sm:space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="budi@domain.com"
                        ...
                        <div>
                            ...
                            <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan') }}" required placeholder="Contoh: Staf Agenda / Kepala Bagian"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:mb-5">
                        ...
                        <div>
                            ...
                            <select name="role" id="role" required
                                ...
                                <option value="pimpinan" {{ old('role') == 'pimpinan' ? 'selected' : '' }}>Pimpinan (Akses Eksekutif)</option>
                                <option value="staf" {{ in_array(old('role'), ['staf', 'staff']) ? 'selected' : '' }}>Staf (Akses Operator)</option>
                            </select>
                            @error('role') 
                                <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p> 
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="nonaktif" {{ old('status') == 'nonaktif' ? 'selected' : '' }}>Nonaktif (Diblokir)</option>
                            </select>
                            @error('status') 
                                <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p> 
                            @enderror
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-1.5">Gunakan kombinasi huruf dan angka agar akun lebih aman.</p>
                        @error('password') 
                            <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p> 
                        @enderror
                    </div>
