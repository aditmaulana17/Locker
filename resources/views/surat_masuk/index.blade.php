...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto">
            ...
            <a href="{{ route('export.surat-masuk.pdf', request()->query()) }}" target="_blank" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:bg-rose-100 sm:px-3.5">
            ...
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        @if($hasFilters)
                            <a href="{{ route('surat-masuk.index') }}" title="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </a>
                        @endif
                    </div>
                        ...
                        <div class="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                            @foreach($kategoris as $k)
                                <label class="group flex h-9 min-w-0 cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 transition hover:border-blue-200 hover:bg-blue-50/60">
                                    <input type="checkbox" name="kategori_id[]" value="{{ $k->id }}" class="kategori-checkbox h-3.5 w-3.5 shrink-0 rounded border-slate-300 text-blue-600 focus:ring-1 focus:ring-blue-500/30" @checked(in_array((string)$k->id, $selectedKategori, true))>
                                    <span class="truncate text-[11px] font-medium text-slate-700 transition group-hover:text-blue-700" title="{{ $k->nama_kategori }}">{{ $k->nama_kategori }}</span>
                                </label>
                            @endforeach
                        </div>
                    @else
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    @forelse($suratMasuks ?? [] as $s)
                        ...
                        <tr class="transition duration-150 hover:bg-slate-50/60">
                            <td class="px-4 py-3.5 text-slate-500 sm:px-6 sm:py-4">{{ optional($s->tanggal_terima)->format('d/m/Y') ?? '-' }}</td>
                            <td class="px-4 py-3.5 text-slate-700 sm:px-6 sm:py-4">
                                <span class="inline-block max-w-[220px] truncate rounded-lg border border-slate-200/60 bg-slate-100 px-2.5 py-1 font-medium text-slate-700">{{ $s->pengirim ?? '-' }}</span>
                            </td>
                            <td class="max-w-xs truncate px-4 py-3.5 font-medium text-slate-800 sm:px-6 sm:py-4" title="{{ $s->perihal }}">{{ $s->perihal }}</td>
                            <td class="px-4 py-3.5 text-slate-500 sm:px-6 sm:py-4">{{ $s->kategori->nama_kategori ?? '-' }}</td>
                            <td class="px-4 py-3.5 sm:px-6 sm:py-4">
                                <span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold {{ $badgeClass }}">{{ ucfirst($status) }}</span>
                            </td>
                            <td class="px-4 py-3.5 text-center sm:px-6 sm:py-4">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('surat-masuk.show', $s) }}" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-blue-50 hover:text-blue-600" title="Lihat Detail & Disposisi">
                                    ...
                                    @if(in_array($userRole, ['admin', 'pimpinan']))
                                        <a href="{{ route('surat-masuk.edit', $s) }}" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-amber-50 hover:text-amber-600" title="Ubah Data">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </a>
                                        <form action="{{ route('surat-masuk.destroy', $s) }}" method="POST" class="delete-form inline">
                                            @csrf
                                            @method('DELETE')
        ...
        @if(isset($suratMasuks) && method_exists($suratMasuks, 'hasPages') && $suratMasuks->hasPages())
            <div class="border-t border-slate-100 px-4 py-3 sm:px-6 sm:py-4">{{ $suratMasuks->withQueryString()->links() }}</div>
        @endif
    </div>
</div>
