                ...
                <div class="overflow-hidden rounded-md border-2 border-slate-400">
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        ...
                        <div class="border-b-2 border-slate-300 p-3 md:border-r-2">
                            <label for="nomor_surat" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        <div class="border-b-2 border-slate-300 p-3">
                            <label for="pengirim" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        {{-- TANGGAL SURAT --}}
                        <div class="border-b-2 border-slate-300 p-3 md:border-r-2">
                            <label for="tanggal_surat" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        {{-- TANGGAL TERIMA --}}
                        <div class="border-b-2 border-slate-300 p-3">
                            <label for="tanggal_terima" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        {{-- KATEGORI --}}
                        <div class="border-b-2 border-slate-300 p-3 md:border-r-2">
                            <label for="kategori_surat_id" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        {{-- STATUS --}}
                        <div class="border-b-2 border-slate-300 p-3">
                            <label for="status" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        <div class="border-b-2 border-slate-300 p-3 md:col-span-2">
                            <label for="perihal" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                        ...
                        <div class="p-3 md:col-span-2">
                            <label for="ringkasan" class="mb-1.5 block text-[11px] font-bold text-slate-700">
                                Ringkasan
                        ...
                        <div class="p-3">
                            ...
                            <div class="mb-2.5 flex items-start gap-1.5 rounded-md border border-blue-200 bg-blue-50 px-2.5 py-2">
                                ...
                                <div class="text-[9px] leading-relaxed">
                                    ...
                                    <p class="text-blue-600">
                                        JPG/PNG akan diproses menjadi JPG terkompresi sebelum disimpan.
                                        PDF tetap PDF.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div id="upload-panel">
                                ...
                                <div
                                    ...
                                    class="mt-2 hidden rounded-md border border-blue-200 bg-blue-50 px-2 py-1.5 text-[9px] text-blue-700"
                                ></div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const compressionStatus = document.getElementById('file-compression-status');
    ...
    /*
     * Untuk kamera:
     * hasil dibuat cukup kecil supaya request POST tidak membengkak.
     */
    const CAMERA_MAX_WIDTH = 1600;
    const CAMERA_MAX_HEIGHT = 1600;
    const CAMERA_TARGET_SIZE = 4.5 * 1024 * 1024;
    const CAMERA_START_QUALITY = 0.82;
    ...
    function setUploadActive() {
        ...
        setActiveButton(btnUpload, true);
        setActiveButton(btnCamera, false);
    ...
    function setCameraActive() {
        ...
        setActiveButton(btnUpload, false);
        setActiveButton(btnCamera, true);
    ...
    async function startCamera() {
        ...
        if (!window.isSecureContext) {
            showCameraError(
                'Kamera membutuhkan HTTPS. Buka website melalui HTTPS.'
            );

            cameraPlaceholder.classList.remove('hidden');
            cameraPlaceholderText.textContent =
                'HTTPS diperlukan untuk mengakses kamera.';
    ...
    function buildCameraImage() {
        ...
        let quality = CAMERA_START_QUALITY;
        let imageData = canvasToDataUrl(
            canvas,
            quality
        );

        while (
            estimateBase64Size(imageData) > CAMERA_TARGET_SIZE &&
            quality > 0.40
        ) {
            quality -= 0.08;

            imageData = canvasToDataUrl(
                canvas,
                quality
            );
        }
        ...
        /*
         * Jika masih besar, turunkan resolusi.
         */
        if (
            estimateBase64Size(imageData) >
            CAMERA_TARGET_SIZE
        ) {
            const resizedCanvas =
                document.createElement('canvas');

            const resizeRatio = 0.75;

            const smallerWidth =
                Math.max(
                    1,
                    Math.round(width * resizeRatio)
                );
    ...
    function handleFileChange() {
        ...
        if (file.size <= 0) {
            ...
            alert(
                'File yang dipilih kosong atau tidak valid.'
            );
        ...
        if (
            ...
            compressionStatus.textContent =
                '✓ Gambar akan dikompres di server sebelum disimpan.';
        } else {
            compressionStatus.textContent =
                '✓ PDF akan disimpan sebagai PDF tanpa konversi gambar.';
        }
    ...
    form.addEventListener(
        ...
        function (event) {
            ...
            /*
             * Jangan izinkan submit bila tidak ada
             * file upload dan tidak ada hasil kamera.
             */
            const hasFile =
                fileInput.files &&
            ...
            /*
             * Jangan kirim dua sumber sekaligus.
             */
            if (hasFile && hasCamera) {
                event.preventDefault();
            ...
            /*
             * Pastikan file tidak lebih dari 10 MB.
             */
            if (hasFile) {
                const file =
                    fileInput.files[0];
    ...
    /*
     * Bila kembali dari validation Laravel dan
     * hasil kamera masih ada.
     */
