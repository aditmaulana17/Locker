...
<div class="mx-auto w-full max-w-4xl px-2 sm:px-3 lg:px-4">
    @if($formErrors->any())
    ...
    <form id="form-surat" method="POST" action="{{ route('surat-masuk.store') }}" enctype="multipart/form-data" novalidate>
        ...
        <div class="overflow-hidden rounded-lg border-2 border-slate-400 bg-white shadow-sm">
            ...
            <div class="border-b-2 border-blue-300 bg-gradient-to-r from-blue-50 to-indigo-50 px-3 py-2.5">
                <div class="flex items-center justify-between gap-2.5">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-[8px] font-bold uppercase tracking-wider text-blue-600">Nomor Agenda Sistem</p>
                            <p class="font-mono text-xs font-bold text-blue-900">{{ $nomorAgenda ?? '-' }}</p>
                        </div>
                    </div>

                    <div class="hidden rounded-md border border-blue-300 bg-white/80 px-2.5 py-1 text-right sm:block">
                        <p class="text-[8px] font-semibold uppercase tracking-wide text-blue-500">Sistem</p>
                        <p class="text-[9px] font-medium text-blue-700">Dibuat otomatis</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-2.5 sm:p-3">
                ...
                <section>
                    <div class="mb-2.5 flex items-start gap-2 border-b-2 border-slate-300 pb-2">
                        ...
                        <div>
                            <h2 class="text-sm font-bold text-slate-800">Informasi Utama Surat</h2>
                            <p class="text-[10px] text-slate-500">Lengkapi identitas dan informasi utama surat masuk.</p>
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden rounded-md border-2 border-slate-400">
                        <div class="grid grid-cols-1 md:grid-cols-2">
                            ...
                            <div class="border-b-2 border-slate-300 p-2.5 md:border-r-2">
                                <label for="nomor_surat" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('nomor_surat'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('nomor_surat') }}</p>
                                @endif
                            </div>

                            {{-- PENGIRIM --}}
                            <div class="border-b-2 border-slate-300 p-2.5">
                                <label for="pengirim" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('pengirim'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('pengirim') }}</p>
                                @endif
                            </div>

                            {{-- TANGGAL SURAT --}}
                            <div class="border-b-2 border-slate-300 p-2.5 md:border-r-2">
                                <label for="tanggal_surat" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('tanggal_surat'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('tanggal_surat') }}</p>
                                @endif
                            </div>

                            {{-- TANGGAL TERIMA --}}
                            <div class="border-b-2 border-slate-300 p-2.5">
                                <label for="tanggal_terima" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('tanggal_terima'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('tanggal_terima') }}</p>
                                @endif
                            </div>

                            {{-- KATEGORI --}}
                            <div class="border-b-2 border-slate-300 p-2.5 md:border-r-2">
                                <label for="kategori_surat_id" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('kategori_surat_id'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('kategori_surat_id') }}</p>
                                @endif
                            </div>

                            {{-- STATUS --}}
                            <div class="border-b-2 border-slate-300 p-2.5">
                                <label for="status" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('status'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('status') }}</p>
                                @endif
                            </div>

                            {{-- PERIHAL --}}
                            <div class="border-b-2 border-slate-300 p-2.5 md:col-span-2">
                                <label for="perihal" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('perihal'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('perihal') }}</p>
                                @endif
                            </div>

                            {{-- RINGKASAN --}}
                            <div class="p-2.5 md:col-span-2">
                                <label for="ringkasan" class="mb-1 block text-[11px] font-bold text-slate-700">
                                ...
                                @if($formErrors->has('ringkasan'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('ringkasan') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    <div class="mb-2.5 flex items-start gap-2 border-b-2 border-slate-300 pb-2">
                        ...
                        <div>
                            <h2 class="text-sm font-bold text-slate-800">Lampiran Dokumen & Arsip Fisik</h2>
                            <p class="text-[10px] text-slate-500">Upload dokumen digital atau scan menggunakan kamera.</p>
                        </div>
                    </div>

                    <div class="mx-auto grid w-full max-w-3xl grid-cols-1 gap-2.5 lg:grid-cols-2">
                        ...
                        <div class="overflow-hidden rounded-md border-2 border-slate-400 bg-white shadow-sm">
                            <div class="border-b-2 border-blue-300 bg-blue-50 px-2.5 py-2">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex min-w-0 items-center gap-2">
                                        ...
                                        <div>
                                            <h3 class="text-[11px] font-bold text-slate-800">Berkas Digital</h3>
                                            <p class="text-[9px] text-slate-500">PDF, JPG, JPEG, PNG</p>
                                        </div>
                                    </div>

                                    <span class="shrink-0 rounded-full border border-rose-200 bg-rose-100 px-1.5 py-0.5 text-[8px] font-bold uppercase text-rose-600">Wajib</span>
                                </div>
                            </div>
                            ...
                            <div class="p-2.5">
                                ...
                                <div class="mb-2 grid grid-cols-2 gap-1">
                                    <button type="button" id="btn-upload" aria-selected="true" class="flex items-center justify-center gap-1 rounded-md border-2 border-blue-600 bg-blue-600 px-2 py-1.5 text-[9px] font-bold text-white shadow-sm transition hover:bg-blue-700">
                                    ...
                                    <button type="button" id="btn-camera" aria-selected="false" class="flex items-center justify-center gap-1 rounded-md border-2 border-slate-400 bg-white px-2 py-1.5 text-[9px] font-bold text-slate-700 transition hover:border-blue-300 hover:bg-blue-50">
                                        <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {{-- UPLOAD PANEL --}}
                                <div id="upload-panel">
                                    <label id="upload-box" for="lampiran_file" class="flex min-h-[105px] cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-slate-400 bg-slate-50 px-2 py-3 text-center transition hover:border-blue-400 hover:bg-blue-50">
                                        ...
                                        <span class="text-[10px] font-bold text-slate-700">Klik untuk memilih file</span>
                                        <span class="mt-0.5 text-[9px] text-slate-500">PDF, JPG, JPEG, PNG</span>
                                        <span class="mt-1 rounded-full bg-blue-100 px-1.5 py-0.5 text-[8px] font-medium text-blue-600">Gambar otomatis dikompres</span>
                                    ...
                                    <div id="selected-file" class="mt-1.5 hidden rounded-md border-2 border-emerald-300 bg-emerald-50 p-1.5">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <p class="text-[9px] font-bold text-emerald-700">File siap diupload</p>
                                                <p id="selected-file-name" class="truncate text-[9px] text-emerald-600"></p>
                                                <p id="selected-file-size" class="text-[8px] text-emerald-500"></p>
                                            </div>

                                            <button type="button" id="clear-file-btn" class="flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-emerald-200 bg-white text-emerald-600 transition hover:bg-emerald-100" aria-label="Hapus file">
                                                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div id="camera-panel" class="hidden">
                                        ...
                                        <div id="camera-container" class="relative aspect-[4/3] w-full">
                                            ...
                                            <div id="camera-placeholder" class="absolute inset-0 flex items-center justify-center bg-slate-900 text-slate-400">
                                                <div class="px-3 text-center">
                                                    <svg class="mx-auto mb-1.5 h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <p id="camera-placeholder-text" class="text-[9px] font-medium">Kamera belum aktif</p>
                                                </div>
                                            </div>

                                            <div id="camera-error" class="absolute inset-x-1.5 bottom-1.5 hidden rounded-md bg-rose-600/95 px-2 py-1 text-center text-[9px] font-medium text-white" role="alert"></div>
                                        </div>
                                    </div>

                                    <div class="mt-1.5 flex flex-wrap justify-center gap-1">
                                        <button type="button" id="start-cam-btn" class="rounded-md border-2 border-blue-600 bg-blue-600 px-2.5 py-1.5 text-[9px] font-bold text-white transition hover:bg-blue-700">
                                            Nyalakan Kamera
                                        </button>

                                        <button type="button" id="capture-btn" class="hidden rounded-md border-2 border-emerald-600 bg-emerald-600 px-2.5 py-1.5 text-[9px] font-bold text-white transition hover:bg-emerald-700">
                                            Ambil Foto
                                        </button>

                                        <button type="button" id="retake-btn" class="hidden rounded-md border-2 border-amber-500 bg-amber-500 px-2.5 py-1.5 text-[9px] font-bold text-white transition hover:bg-amber-600">
                                            Foto Ulang
                                        </button>

                                        <button type="button" id="stop-cam-btn" class="hidden rounded-md border-2 border-rose-600 bg-rose-600 px-2.5 py-1.5 text-[9px] font-bold text-white transition hover:bg-rose-700">
                                            Tutup Kamera
                                        </button>
                                    </div>
                                    ...
                                    <div id="snapshot-preview" class="{{ old('captured_image') ? '' : 'hidden' }} mt-1.5 rounded-md border-2 border-emerald-300 bg-emerald-50 px-2 py-1 text-center text-[9px] font-bold text-emerald-600">
                                        ✓ Hasil scan berhasil diambil.
                                    </div>
                                </div>

                                @if($formErrors->has('lampiran_file'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('lampiran_file') }}</p>
                                @endif

                                @if($formErrors->has('captured_image'))
                                    <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('captured_image') }}</p>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="border-b-2 border-indigo-300 bg-indigo-50 px-2.5 py-2">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex min-w-0 items-center gap-2">
                                        ...
                                        <div>
                                            <h3 class="text-[11px] font-bold text-slate-800">Lokasi Arsip Fisik</h3>
                                            <p class="text-[9px] text-slate-500">Lokasi penyimpanan dokumen fisik</p>
                                        </div>
                                    </div>

                                    <span class="shrink-0 rounded-full border border-slate-300 bg-white px-1.5 py-0.5 text-[8px] font-bold uppercase text-slate-500">Opsional</span>
                                </div>
                            </div>

                            <div class="flex min-h-[210px] flex-col p-2.5">
                                <div class="flex flex-1 flex-col justify-center rounded-md border-2 border-dashed border-slate-400 bg-slate-50 p-2.5">
                                    <div class="mx-auto w-full max-w-xs text-center">
                                        ...
                                        <h4 class="text-[11px] font-bold text-slate-700">Lokasi Penyimpanan</h4>
                                        ...
                                        <div class="mt-2.5 text-left">
                                            <label for="lokasi_arsip_fisik" class="mb-1 block text-[10px] font-bold text-slate-700">
                                            ...
                                            @if($formErrors->has('lokasi_arsip_fisik'))
                                                <p class="mt-1 text-[10px] font-medium text-rose-600">{{ $formErrors->first('lokasi_arsip_fisik') }}</p>
                                            @endif
                                        </div>

                                        <div class="mt-2 rounded-md border-2 border-slate-300 bg-white px-2 py-1.5 text-left">
                                            <div class="flex items-start gap-1">
                                                <svg class="mt-0.5 h-3 w-3 shrink-0 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M12 21a9 9 0 100-18 9 9 0 000 18z"/>
                                                </svg>
            ...
            {{-- FOOTER --}}
            <div class="flex flex-col-reverse gap-1.5 border-t-2 border-slate-400 bg-slate-50 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-end">
                <a href="{{ route('surat-masuk.index') }}" class="inline-flex w-full items-center justify-center rounded-md border-2 border-slate-400 bg-white px-3.5 py-1.5 text-[10px] font-bold text-slate-700 transition hover:bg-slate-100 sm:w-auto">
                    Batal
                </a>

                <button id="submit-btn" type="submit" class="inline-flex w-full items-center justify-center rounded-md border-2 border-blue-600 bg-blue-600 px-3.5 py-1.5 text-[10px] font-bold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setUploadActive() {
        ...
        btnUpload.classList.remove('border-slate-400', 'bg-white', 'text-slate-700');
        btnUpload.classList.add('border-blue-600', 'bg-blue-600', 'text-white');

        btnCamera.classList.remove('border-blue-600', 'bg-blue-600', 'text-white');
        btnCamera.classList.add('border-slate-400', 'bg-white', 'text-slate-700');
    ...
    function setCameraActive() {
        ...
        btnCamera.classList.remove('border-slate-400', 'bg-white', 'text-slate-700');
        btnCamera.classList.add('border-blue-600', 'bg-blue-600', 'text-white');

        btnUpload.classList.remove('border-blue-600', 'bg-blue-600', 'text-white');
        btnUpload.classList.add('border-slate-400', 'bg-white', 'text-slate-700');
    ...
    function capturePhoto() {
        if (!cameraStream || !video.videoWidth || !video.videoHeight) {
            showCameraError('Kamera belum siap. Tunggu beberapa saat lalu coba lagi.');
            return;
        }
        ...
        snapshotPreview.textContent = '✓ Hasil scan berhasil diambil dan siap disimpan.';
    ...
    // Jika halaman dikembalikan oleh validation Laravel
    // dan captured_image masih tersedia, tampilkan panel kamera.
    if (capturedImage.value) {
        ...
        imagePreview.src = capturedImage.value;
        imagePreview.hidden = false;

        snapshotPreview.classList.remove('hidden');
        snapshotPreview.textContent = '✓ Hasil scan sebelumnya masih tersedia.';
    }
});
</script>
@endsection