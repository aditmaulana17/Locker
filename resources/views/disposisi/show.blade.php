...
<div class="max-w-6xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-4">
            <a href="{{ route('disposisi.index') }}" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition shrink-0" title="Kembali ke Daftar Disposisi">
        ...
        <div class="flex items-center gap-2">
            ...
            @if(auth()->user()->isAdmin() || auth()->id() === $disposisi->dari_user_id)
                <a href="{{ route('disposisi.edit', $disposisi) }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-xl transition shadow-xs">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-xs border border-slate-200/80 p-6 sm:p-8 space-y-6">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                ...
                @endphp
                <span class="px-3.5 py-1 rounded-full text-xs font-bold border capitalize shrink-0 {{ $statusClasses[$st] ?? 'bg-slate-50 text-slate-600 border-slate-200' }}">
                    {{ $disposisi->status }}
                </span>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="min-w-0 bg-slate-50/60 p-4 rounded-xl border border-slate-100">
                    ...
                    <p class="mt-1 text-xs sm:text-sm font-bold text-slate-800 break-words">
                        {{ $disposisi->dari?->name ?? $disposisi->pengirim ?? '-' }}
                    </p>
                    @if(isset($disposisi->dari?->jabatan))
                        <p class="text-[11px] text-slate-500 font-medium">{{ $disposisi->dari->jabatan }}</p>
                    @endif
                </div>
                ...
                <div class="min-w-0 bg-slate-50/60 p-4 rounded-xl border border-slate-100">
                    ...
                    <p class="mt-1 text-xs sm:text-sm font-bold text-slate-800 break-words">
                        {{ $disposisi->kepada?->name ?? $disposisi->penerima ?? $disposisi->tujuan ?? '-' }}
                    </p>
                    @if(isset($disposisi->kepada?->jabatan))
                        <p class="text-[11px] text-slate-500 font-medium">{{ $disposisi->kepada->jabatan }}</p>
                    @endif
                </div>
                ...
                <div class="min-w-0 bg-slate-50/60 p-4 rounded-xl border border-slate-100">
                    ...
                    <p class="mt-1 text-xs sm:text-sm font-bold text-slate-800">
                        {{ $disposisi->batas_waktu ? \Carbon\Carbon::parse($disposisi->batas_waktu)->translatedFormat('d F Y') : '-' }}
                    </p>
                </div>
                ...
                <div class="min-w-0 bg-slate-50/60 p-4 rounded-xl border border-slate-100">
                    ...
                    <p class="mt-1 text-xs sm:text-sm font-bold text-slate-800">
                        {{ $disposisi->created_at ? $disposisi->created_at->translatedFormat('d F Y H:i') : '-' }}
                    </p>
                </div>
            </div>
            ...
            <div class="space-y-2 pt-2">
                ...
                <div class="p-4 sm:p-5 bg-slate-50 rounded-xl border border-slate-200/80 text-xs sm:text-sm text-slate-800 leading-relaxed whitespace-pre-line break-words shadow-2xs font-medium">
                    {{ $disposisi->instruksi ?? $disposisi->catatan ?? $disposisi->isi_disposisi ?? 'Tidak ada instruksi atau catatan khusus.' }}
                </div>
            </div>

            <!-- Form Update Status Cepat (Akses khusus Penerima / Admin) -->
            @if(auth()->user()->isAdmin() || auth()->id() === $disposisi->kepada_user_id)
                <div class="pt-6 border-t border-slate-100">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">Perbarui Status Pekerjaan</label>
                    <form action="{{ route('disposisi.status', $disposisi) }}" method="POST" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        @csrf
                        @method('PATCH')
                        <select name="status" class="w-full sm:w-auto rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-2.5 px-4 transition text-slate-800 shadow-2xs font-semibold">
                            <option value="menunggu" @selected($st === 'menunggu')>Menunggu</option>
                            <option value="diproses" @selected($st === 'diproses' || $st === 'proses')>Diproses</option>
                    ...
                    @if($disposisi->suratMasuk)
                        <div class="space-y-3.5">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs font-semibold text-slate-700 break-all mt-0.5">{{ $disposisi->suratMasuk->nomor_surat ?? '-' }}</p>
                            </div>
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs font-semibold text-slate-700 break-words mt-0.5">
                                    {{ $disposisi->suratMasuk->pengirim ?? ($disposisi->suratMasuk->instansi->nama_instansi ?? '-') }}
                                </p>
                            </div>
                        </div>
