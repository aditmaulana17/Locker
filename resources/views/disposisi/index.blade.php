...
@php
    ...
    }catch(\Throwable $e){$dateRangeValue='';}
    ...
    $getTanggalDisposisi=function($disposisi){
        ...
        if(!$tanggal)return '-';
        try{return \Carbon\Carbon::parse($tanggal)->format('d/m/Y');}
...
.archive-date-panel,.archive-status-panel{border:1px solid #cbd5e1;background:#fff;box-shadow:0 18px 45px rgba(15,23,42,.14),0 6px 18px rgba(15,23,42,.07)}
.archive-date-panel{position:absolute;top:calc(100% + 8px);left:0;z-index:9999;width:620px;max-width:calc(100vw - 24px);overflow:hidden;border-radius:16px}
.archive-date-panel.hidden,.archive-status-panel.hidden,.archive-picker-view.hidden{display:none}
.archive-date-header{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 12px;border-bottom:1px solid #cbd5e1;background:#f8fafc}
.archive-date-header-title{color:#334155;font-size:12px;font-weight:800}
.archive-date-nav{display:inline-flex;width:32px;height:32px;align-items:center;justify-content:center;flex:none;border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#64748b;cursor:pointer;transition:.15s}
.archive-date-nav:hover{border-color:#93c5fd;background:#eff6ff;color:#2563eb}
.archive-date-months{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}
.archive-date-month{min-width:0;padding:12px;border-right:1px solid #cbd5e1}
.archive-date-month:last-child{border-right:0}
.archive-month-header{display:flex;align-items:center;justify-content:center;gap:2px;margin-bottom:8px}
.archive-month-header button{display:inline-flex;align-items:center;gap:2px;padding:5px 6px;border:0;border-radius:8px;background:transparent;color:#334155;font-size:11px;font-weight:800;cursor:pointer;transition:.15s}
.archive-month-header button:hover{background:#eff6ff;color:#2563eb}
...
.archive-calendar-day{display:flex;width:100%;height:31px;align-items:center;justify-content:center;border:0;border-radius:8px;background:transparent;color:#475569;font-size:10px;font-weight:600;cursor:pointer;transition:.15s}
.archive-calendar-day:hover{background:#eff6ff;color:#2563eb}
...
.archive-calendar-day.start-date{border-radius:999px 6px 6px 999px;background:#2563eb;color:#fff}
.archive-calendar-day.end-date{border-radius:6px 999px 999px 6px;background:#2563eb;color:#fff}
...
.archive-date-footer{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 12px 11px;border-top:1px solid #cbd5e1}
...
.archive-date-btn{min-height:32px;padding:0 11px;border-radius:9px;font-size:9px;font-weight:800;cursor:pointer;transition:.15s}
.archive-date-btn.secondary{border:1px solid #cbd5e1;background:#f8fafc;color:#475569}
...
.archive-picker-item{display:flex;min-height:36px;align-items:center;justify-content:center;border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#475569;font-size:10px;font-weight:700;cursor:pointer;transition:.15s}
...
.archive-status-panel{position:absolute;top:calc(100% + 8px);right:0;z-index:9999;width:370px;overflow:hidden;border-radius:16px}
.archive-status-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid #cbd5e1;background:#f8fafc}
...
.archive-status-count{display:inline-flex;min-height:25px;align-items:center;justify-content:center;padding:0 9px;border:1px solid #fde68a;border-radius:999px;background:#fffbeb;color:#d97706;font-size:9px;font-weight:800}
.archive-status-actions{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px 14px;border-bottom:1px solid #e2e8f0}
...
.archive-status-options{max-height:210px;overflow-y:auto;padding:9px 14px 11px}
.archive-status-item{display:flex;min-height:43px;align-items:center;gap:9px;margin-bottom:6px;padding:8px 10px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;cursor:pointer;transition:.15s}
.archive-status-item:last-child{margin-bottom:0}
.archive-status-item:hover{border-color:#93c5fd;background:#eff6ff}
.archive-status-item input{width:16px;height:16px;flex:none;accent-color:#2563eb}
...
.archive-status-footer{display:flex;justify-content:flex-end;padding:9px 14px;border-top:1px solid #cbd5e1;background:#f8fafc}
.archive-status-close{min-height:32px;padding:0 12px;border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#475569;font-size:9px;font-weight:800;cursor:pointer}
.archive-status-close:hover{background:#f1f5f9}
.disposition-table-wrapper{overflow:hidden;border:1px solid #64748b;border-radius:16px;background:#fff;box-shadow:0 1px 3px rgba(15,23,42,.06),0 8px 24px rgba(15,23,42,.04)}
...
.disposition-table thead tr{border-bottom:2px solid #64748b}
.disposition-table thead th{padding:13px 16px;border-right:1px solid #94a3b8;border-bottom:2px solid #64748b;color:#334155;font-size:10px;font-weight:800;letter-spacing:.05em;line-height:1.4;text-align:left;text-transform:uppercase;vertical-align:middle}
...
.disposition-table tbody td{padding:14px 16px;border-right:1px solid #cbd5e1;border-bottom:1px solid #cbd5e1;color:#475569;font-size:12px;line-height:1.5;vertical-align:middle}
...
.disposition-table .receiver-box{display:flex;min-width:150px;max-width:210px;flex-direction:column;gap:1px;padding:6px 9px;border:1px solid #cbd5e1;border-radius:9px;background:#f1f5f9}
.disposition-table .receiver-name{overflow:hidden;color:#334155;font-weight:700;text-overflow:ellipsis;white-space:nowrap}
.disposition-table .receiver-position{overflow:hidden;color:#94a3b8;font-size:10px;text-overflow:ellipsis;white-space:nowrap}
.disposition-table .status-badge{display:inline-flex;align-items:center;border-radius:999px;border-width:1px;padding:5px 10px;font-size:10px;font-weight:800}
.disposition-table .action-cell{width:125px}
.disposition-table .action-buttons{display:inline-flex;align-items:center;justify-content:center;gap:3px}
.disposition-table .action-button{display:inline-flex;width:30px;height:30px;align-items:center;justify-content:center;border-radius:8px;color:#64748b;transition:.15s}
...
.disposition-table-empty{padding:48px 16px;text-align:center}
@media(max-width:767px){
    ...
    .archive-date-month{border-right:0;border-bottom:1px solid #cbd5e1}
    ...
    <div class="rounded-2xl border border-slate-300 bg-white p-3 shadow-sm sm:p-4">
        <form id="filterForm" method="GET" action="{{ route('disposisi.index') }}" class="space-y-3">
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                <div class="lg:col-span-5">
                    <div class="relative">
                        ...
                        <input id="search" name="search" type="text" value="{{ request('search') }}" placeholder="Cari nomor surat, penerima atau isi instruksi..." autocomplete="off" class="h-11 w-full rounded-xl border border-slate-300 bg-slate-50 pl-9 pr-3 text-xs text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 sm:text-sm">
                    </div>
                </div>
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        @if($hasFilters)
                            <a href="{{ route('disposisi.index') }}" title="Reset Filter" aria-label="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </a>
                        @endif
                    </div>
            ...
            <div class="archive-status-dropdown">
                <button type="button" id="statusDropdownButton" class="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-300 bg-white px-3.5 py-3 text-left transition hover:border-slate-400 hover:bg-slate-50 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        <span id="statusCount" class="inline-flex min-h-7 items-center rounded-full border border-amber-100 bg-amber-50 px-2.5 text-[10px] font-bold text-amber-600">{{ count($selectedStatus) }} dipilih</span>
                ...
                <div id="statusDropdownPanel" class="archive-status-panel hidden">
                    <div class="archive-status-head">
                        ...
                        <span id="statusPanelCount" class="archive-status-count">{{ count($selectedStatus) }} dipilih</span>
                    </div>
                    ...
                    @forelse($disposisis??[] as $d)
                        @php
                            ...
                            if($batasWaktu!=='-'){
                                ...
                                }catch(\Throwable $e){$isLate=false;}
                            }
                        @endphp
                        <tr>
                            ...
                            <td class="cell-status"><span class="status-badge {{ $badgeClass }}">{{ $statusOptions[$status]??ucfirst($status) }}</span></td>
                            ...
                            <td class="action-cell">
                                <div class="action-buttons">
                                    ...
                                    @if(in_array($userRole,['admin','pimpinan'],true)&&Route::has('disposisi.edit'))
                                        <a href="{{ route('disposisi.edit',$d) }}" title="Ubah Disposisi" aria-label="Ubah disposisi" class="action-button edit">
                                    ...
                                    @if(in_array($userRole,['admin','pimpinan'],true)&&Route::has('disposisi.destroy'))
                                        <form action="{{ route('disposisi.destroy',$d) }}" method="POST" class="delete-form inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" title="Hapus Disposisi" aria-label="Hapus disposisi" class="action-button delete delete-btn">
...
(function(){
    ...
    function cloneDate(date){return date?new Date(date.getFullYear(),date.getMonth(),date.getDate()):null}
    ...
    function formatDisplay(date){return date?`${pad(date.getDate())}/${pad(date.getMonth()+1)}/${date.getFullYear()}`:''}
    function formatRange(start,end){return start&&end?`${formatDisplay(start)} - ${formatDisplay(end)}`:''}
    function isSameDay(first,second){return !!(first&&second&&dateKey(first)===dateKey(second))}
    function isBefore(first,second){return dateKey(first)<dateKey(second)}
    function isAfter(first,second){return dateKey(first)>dateKey(second)}
    ...
    function updateDateInfo(){
        if(tempStart&&tempEnd)dateInfo.textContent=formatRange(tempStart,tempEnd);
        else if(tempStart)dateInfo.textContent=`${formatDisplay(tempStart)} - pilih tanggal akhir`;
        else dateInfo.textContent='Pilih tanggal mulai';
    }
    ...
    function buildDayButton(date,otherMonth){
        ...
        if(tempStart&&tempEnd&&!isBefore(date,tempStart)&&!isAfter(date,tempEnd))classes.push('in-range');
    ...
    function renderCalendar(year,month){
        let html=`<div class="archive-date-month"><div class="archive-month-header">
            <button type="button" data-action="month" data-month="${month}" data-year="${year}">${months[month]}
                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </button>
            <button type="button" data-action="year" data-month="${month}" data-year="${year}">${year}
                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </button>
        </div><div class="archive-month-grid">`;

        weekdays.forEach(day=>html+=`<div class="archive-calendar-weekday">${day}</div>`);
        ...
        for(let index=firstWeekday-1;index>=0;index--)html+=buildDayButton(new Date(year,month-1,daysInPreviousMonth-index),true);
        for(let day=1;day<=daysInMonth;day++)html+=buildDayButton(new Date(year,month,day),false);

        const totalDays=firstWeekday+daysInMonth;
        const remaining=(7-totalDays%7)%7;
        for(let day=1;day<=remaining;day++)html+=buildDayButton(new Date(year,month+1,day),true);
    ...
    function renderCalendarView(){
        ...
        dateContent.innerHTML=renderCalendar(firstYear,firstMonth)+renderCalendar(secondDate.getFullYear(),secondDate.getMonth());
        bindCalendarEvents();
        updateDateInfo();
    }
    ...
    function renderMonthPicker(){
        ...
        monthPickerView.innerHTML=`
            <div class="archive-picker-toolbar"><span>${pickerYear}</span></div>
            <div class="archive-picker-grid">
                ${months.map((month,index)=>`<button type="button" class="archive-picker-item ${index===pickerMonth?'active':''}" data-picker-month="${index}">${month}</button>`).join('')}
            </div>`;
    ...
    function renderYearPicker(){
        ...
        let html=`<div class="archive-year-toolbar">
            <div class="archive-year-nav"><button type="button" id="yearPrev">‹</button></div>
            <span class="archive-year-range">${startYear} - ${endYear}</span>
            <div class="archive-year-nav"><button type="button" id="yearNext">›</button></div>
        </div><div class="archive-picker-grid">`;

        for(let year=startYear;year<=endYear;year++)html+=`<button type="button" class="archive-picker-item ${year===pickerYear?'active':''}" data-picker-year="${year}">${year}</button>`;
    ...
    function applyDateRange(){
        if(!tempStart||!tempEnd){
            showWarning('Tanggal belum lengkap','Pilih tanggal mulai dan tanggal akhir terlebih dahulu.');
            return;
        }
    ...
    dateInput?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
        datePanel.classList.contains('hidden')?openDatePicker():closeDatePicker();
    });
    ...
    function updateStatusSummary(){
        const checked=statusCheckboxes
            ...
            .map(checkbox=>{
                const label=checkbox.parentElement?.querySelector('.archive-status-item-label');
        ...
        statusSummary.textContent=checked.slice(0,2).join(', ')+(count>2?` +${count-2}`:'');
    }

    statusDropdownButton?.addEventListener('click',function(event){
        event.preventDefault();
        event.stopPropagation();
        statusDropdownPanel.classList.contains('hidden')?openStatusDropdownPanel():closeStatusDropdownPanel();
    });

    statusCheckboxes.forEach(checkbox=>checkbox.addEventListener('change',updateStatusSummary));
    ...
    document.addEventListener('click',function(event){
        if(!event.target.closest('.archive-date-picker'))closeDatePicker();
        if(!event.target.closest('.archive-status-dropdown'))closeStatusDropdownPanel();
    });
    ...
    document.getElementById('filterForm')?.addEventListener('submit',function(event){
        ...
        if(!start||!end){
            event.preventDefault();
            showWarning('Rentang tanggal belum lengkap','Pilih tanggal mulai dan tanggal akhir.');
            return;
        }

        if(isAfter(start,end)){
            event.preventDefault();
            showWarning('Rentang tanggal tidak valid','Tanggal mulai tidak boleh lebih besar dari tanggal akhir.');
        }
    });
