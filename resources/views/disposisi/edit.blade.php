...
<div class="max-w-4xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-4">
            <a href="{{ route('disposisi.index') }}" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition shrink-0" title="Kembali">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                <nav class="flex items-center gap-2 text-xs text-slate-400 font-medium mb-1">
                    <a href="{{ route('disposisi.index') }}" class="hover:text-slate-600 transition">Disposisi</a>
                    <span>/</span>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('disposisi.index') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition">
                <span>Batal</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xs border border-slate-200/80 overflow-hidden">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="space-y-1 min-w-0 flex-1">
                    ...
                    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-300 font-medium pt-0.5">
                        <span>No. Surat: <strong class="text-white">{{ $disposisi->suratMasuk->nomor_surat ?? '-' }}</strong></span>
                        <span class="text-slate-500">&bull;</span>
                        <span>Pengirim: <strong class="text-white">{{ $disposisi->suratMasuk->pengirim ?? '-' }}</strong></span>
                        <a href="{{ route('surat-masuk.show', $disposisi->suratMasuk) }}" target="_blank" class="inline-flex items-center gap-1 text-xs text-indigo-300 hover:text-indigo-200 font-semibold ml-auto transition">
        ...
        <!-- Form Edit Disposisi -->
        <form action="{{ route('disposisi.update', $disposisi->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                @if(isset($users) && count($users) > 0)
                    <select id="kepada_user_id" name="kepada_user_id" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-3 px-4 transition text-slate-800 shadow-2xs font-medium">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" @selected(old('kepada_user_id', $disposisi->kepada_user_id ?? $disposisi->kepada?->id) == $u->id)>
                ...
                @else
                    <input type="text" id="penerima" name="penerima" 
                        value="{{ old('penerima', $disposisi->penerima ?? $disposisi->tujuan ?? $disposisi->kepada?->name) }}" required 
            ...
            <div class="space-y-2">
                ...
                <textarea id="instruksi" name="instruksi" rows="3" required placeholder="Contoh: Mohon ditindaklanjuti segera dan koordinasikan dengan tim terkait." class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm p-4 transition text-slate-800 placeholder-slate-400 shadow-2xs leading-relaxed">{{ old('instruksi', $disposisi->instruksi ?? $disposisi->isi_disposisi) }}</textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="catatan" name="catatan" rows="2" placeholder="Tuliskan catatan khusus atau petunjuk teknis tambahan..." class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm p-4 transition text-slate-800 placeholder-slate-400 shadow-2xs leading-relaxed">{{ old('catatan', $disposisi->catatan) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                ...
                <div class="space-y-2">
                    ...
                    <select id="sifat" name="sifat" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-3 px-4 transition text-slate-800 shadow-2xs font-medium">
                        @foreach(['biasa' => 'Biasa', 'penting' => 'Penting', 'segera' => 'Segera', 'rahasia' => 'Rahasia'] as $key => $label)
                            <option value="{{ $key }}" @selected(old('sifat', strtolower($disposisi->sifat ?? 'biasa')) == $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                @endif
                ...
                <div class="space-y-2">
                    ...
                    @endphp
                    <input type="date" id="batas_waktu" name="batas_waktu" value="{{ $formattedDate }}" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-3 px-4 transition text-slate-800 shadow-2xs font-medium">
                </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        @foreach(['menunggu' => 'Menunggu', 'diproses' => 'Diproses', 'selesai' => 'Selesai'] as $stKey => $stLabel)
                            <label class="flex items-center justify-center p-3 border rounded-xl text-xs sm:text-sm font-semibold cursor-pointer transition has-[:checked]:bg-purple-50 has-[:checked]:border-purple-500 has-[:checked]:text-purple-700 text-slate-600 bg-slate-50/50 border-slate-200 hover:bg-slate-100">
                                <input type="radio" name="status" value="{{ $stKey }}" @checked(old('status', strtolower($disposisi->status ?? 'menunggu')) == $stKey) class="hidden">
                                <span>{{ $stLabel }}</span>
                            </label>
                        @endforeach
                    </div>
            ...
            <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
                <a href="{{ route('disposisi.index') }}" class="w-full sm:w-auto text-center px-5 py-2.5 text-xs font-semibold rounded-xl text-slate-700 bg-slate-100 hover:bg-slate-200 transition">
                    Batal
                </a>
