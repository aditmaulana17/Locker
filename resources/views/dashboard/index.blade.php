...
<div class="space-y-6 sm:space-y-8">
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl sm:rounded-3xl p-5 sm:p-8 text-white shadow-xl shadow-blue-500/10">
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5 sm:gap-6">
            <div class="space-y-2">
                ...
                <h1 class="text-xl sm:text-3xl font-bold tracking-tight leading-snug">Selamat Datang Kembali, {{ auth()->user()->name ?? 'Pengguna' }}!</h1>
            ...
            <div class="flex items-center gap-2.5 sm:gap-3 shrink-0">
                @if(!$isStaf)
                    <a href="{{ route('surat-masuk.create') }}" class="flex-1 sm:flex-none justify-center inline-flex items-center gap-2 px-3.5 sm:px-4 py-2.5 bg-white text-blue-700 hover:bg-blue-50 rounded-xl text-xs font-semibold shadow-sm transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        <span>Surat Masuk</span>
                    </a>
                    <a href="{{ route('surat-keluar.create') }}" class="flex-1 sm:flex-none justify-center inline-flex items-center gap-2 px-3.5 sm:px-4 py-2.5 bg-blue-500/30 hover:bg-blue-500/40 text-white border border-white/20 rounded-xl text-xs font-semibold backdrop-blur-md transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        <span>Surat Keluar</span>
                    </a>
                @else
                    <a href="{{ route('disposisi.index') }}" class="flex-1 sm:flex-none justify-center inline-flex items-center gap-2 px-4 py-2.5 bg-white text-blue-700 hover:bg-blue-50 rounded-xl text-xs font-semibold shadow-sm transition">
    ...
    <!-- STATS GRID CARDS -->
    <div class="grid grid-cols-2 {{ !$isStaf ? 'lg:grid-cols-2 xl:grid-cols-4' : 'lg:grid-cols-2' }} gap-3 sm:gap-5">
        ...
        @if(!$isStaf)
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $totalSuratMasuk ?? 0 }}</span>
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $totalSuratKeluar ?? 0 }}</span>
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $suratPending ?? 0 }}</span>
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $suratSelesai ?? 0 }}</span>
        ...
        @if($isStaf)
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $disposisiMenunggu ?? 0 }}</span>
            ...
            <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm hover:shadow-md transition group">
                ...
                <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
                    <span class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">{{ $disposisiSelesai ?? 0 }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div class="{{ !$isStaf ? 'lg:col-span-1' : 'lg:col-span-3' }} bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-sm flex flex-col justify-between">
            <div>
                <!-- Header Widget -->
                <div class="flex items-center justify-between pb-4 border-b border-slate-100">
                    ...
                    <a href="{{ route('disposisi.index') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700 hover:underline">Lihat Semua →</a>
                </div>
                ...
                <div class="mt-4 space-y-3">
                    @forelse($listDisposisi ?? [] as $d)
                        ...
                        <a href="{{ route('disposisi.show', $d->id) }}" class="group relative block p-4 bg-white hover:bg-slate-50/80 rounded-xl border border-slate-200/70 {{ $currentConfig['border'] }} border-l-4 shadow-xs hover:shadow-sm transition-all space-y-2">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <span class="text-[10px] {{ $currentConfig['bg'] }} px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wide shrink-0 border">
                            ...
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
                                <span class="inline-flex items-center gap-1">
                                    ...
                                    Dari: <strong class="text-slate-700">{{ $d->dari->name ?? '-' }}</strong>
                                </span>
            ...
            <div class="pt-4 mt-4 border-t border-slate-100">
                <a href="{{ route('disposisi.index') }}" class="w-full py-2.5 px-4 text-center block text-xs font-bold text-slate-700 bg-slate-100 hover:bg-blue-600 hover:text-white rounded-xl shadow-xs transition-all duration-200">
    ...
    @if(!$isStaf)
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
            <div class="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between gap-2">
                ...
                <a href="{{ route('surat-masuk.index') }}" class="px-3 py-1.5 text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition shrink-0">
                    Lihat Semua
                </a>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
                        @forelse($suratMasukTerbaru ?? [] as $sm)
                            <tr class="hover:bg-slate-50/60 transition">
                                <td class="py-3.5 px-4 sm:px-6 font-bold text-blue-600 font-mono">{{ $sm->nomor_agenda ?? '-' }}</td>
                                <td class="py-3.5 px-4 sm:px-6 font-semibold text-slate-800 max-w-xs truncate">{{ $sm->perihal ?? '-' }}</td>
                                <td class="py-3.5 px-4 sm:px-6">{{ $sm->instansi->nama_instansi ?? '-' }}</td>
                                <td class="py-3.5 px-4 sm:px-6">{{ $sm->kategori->nama_kategori ?? '-' }}</td>
                                <td class="py-3.5 px-4 sm:px-6">
                                    ...
                                    @endphp
                                    <span class="px-2.5 py-0.5 sm:py-1 rounded-full text-[10px] font-bold border {{ $badgeStyle }}">
