<?php $__env->startSection('title', 'Buat Disposisi Surat'); ?>
...
<div class="max-w-4xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div class="flex items-center gap-4">
            <a href="<?php echo e(route('surat-masuk.show', $suratMasuk)); ?>" class="w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition shrink-0" title="Kembali ke Detail Surat">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                <nav class="flex items-center gap-2 text-xs text-slate-400 font-medium mb-1">
                    <a href="<?php echo e(route('surat-masuk.index')); ?>" class="hover:text-slate-600 transition">Surat Masuk</a>
                    <span>/</span>
                    <a href="<?php echo e(route('surat-masuk.show', $suratMasuk)); ?>" class="hover:text-slate-600 transition">Detail</a>
                    <span>/</span>
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('surat-masuk.show', $suratMasuk)); ?>" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition">
                <span>Kembali</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xs border border-slate-200/80 overflow-hidden">
                ...
                <div class="space-y-1 min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-white/10 text-indigo-200 border border-white/10">
                            #<?php echo e($suratMasuk->nomor_agenda ? 'AG/' . $suratMasuk->nomor_agenda : $suratMasuk->id); ?>
                    ...
                    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-300 font-medium pt-0.5">
                        <span>No. Surat: <strong class="text-white"><?php echo e($suratMasuk->nomor_surat ?? '-'); ?></strong></span>
                        <span class="text-slate-500">&bull;</span>
                        <span>Pengirim: <strong class="text-white"><?php echo e($suratMasuk->pengirim ?? ($suratMasuk->instansi->nama_instansi ?? '-')); ?></strong></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Form Disposisi -->
        <form method="POST" action="<?php echo e(route('disposisi.store')); ?>" class="p-6 sm:p-8 space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="space-y-2">
                ...
                <?php if(isset($users) && count($users) > 0): ?>
                    <select id="kepada_user_id" name="kepada_user_id" required class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-3 px-4 transition text-slate-800 shadow-2xs font-medium">
                        ...
                        <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($u->id); ?>" <?php if(old('kepada_user_id') == $u->id): echo 'selected'; endif; ?>>
                ...
                <?php else: ?>
                    <input type="text" id="penerima" name="penerima" 
                        value="<?php echo e(old('penerima')); ?>" required 
            ...
            <div class="space-y-2">
                ...
                <textarea id="instruksi" name="instruksi" rows="3" required placeholder="Contoh: Mohon ditindaklanjuti segera dan koordinasikan dengan tim terkait." class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm p-4 transition text-slate-800 placeholder-slate-400 shadow-2xs leading-relaxed"><?php echo e(old('instruksi')); ?></textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="catatan" name="catatan" rows="2" placeholder="Tuliskan catatan khusus atau petunjuk teknis tambahan..." class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm p-4 transition text-slate-800 placeholder-slate-400 shadow-2xs leading-relaxed"><?php echo e(old('catatan')); ?></textarea>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" id="batas_waktu" name="batas_waktu" value="<?php echo e(old('batas_waktu')); ?>" class="w-full rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 text-xs sm:text-sm py-3 px-4 transition text-slate-800 shadow-2xs font-medium">
                </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <?php $__currentLoopData = ['menunggu' => 'Menunggu', 'diproses' => 'Diproses', 'selesai' => 'Selesai']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stKey => $stLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <label class="flex items-center justify-center p-3 border rounded-xl text-xs sm:text-sm font-semibold cursor-pointer transition has-[:checked]:bg-purple-50 has-[:checked]:border-purple-500 has-[:checked]:text-purple-700 text-slate-600 bg-slate-50/50 border-slate-200 hover:bg-slate-100">
                                <input type="radio" name="status" value="<?php echo e($stKey); ?>" <?php if(old('status', 'menunggu') == $stKey): echo 'checked'; endif; ?> class="hidden">
            ...
            <div class="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
                <a href="<?php echo e(route('surat-masuk.show', $suratMasuk)); ?>" class="w-full sm:w-auto text-center px-5 py-2.5 text-xs font-semibold rounded-xl text-slate-700 bg-slate-100 hover:bg-slate-200 transition">
                    Batal
                </a>
