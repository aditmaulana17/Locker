...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto">
            <?php if(Route::has('export.disposisi.excel')): ?>
                <a href="<?php echo e(route('export.disposisi.excel', request()->query())); ?>" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs font-semibold text-emerald-700 shadow-sm transition hover:bg-emerald-100 sm:px-3.5">
            ...
            <?php if(Route::has('export.disposisi.pdf')): ?>
                <a href="<?php echo e(route('export.disposisi.pdf', request()->query())); ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:bg-rose-100 sm:px-3.5">
            ...
            <?php if(in_array($userRole, ['admin','pimpinan'], true) && Route::has('surat-masuk.index')): ?>
                <a href="<?php echo e(route('surat-masuk.index')); ?>" title="Pilih surat masuk untuk membuat disposisi" class="inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-2.5 py-2 text-xs font-semibold text-white shadow-md shadow-blue-600/20 transition hover:bg-blue-700 sm:px-4">
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm sm:p-5">
        <form id="filterForm" method="GET" action="<?php echo e(route('disposisi.index')); ?>" class="space-y-3">
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                <div class="lg:col-span-5">
                    <div class="relative">
                        ...
                        <input id="search" name="search" type="text" value="<?php echo e(request('search')); ?>" placeholder="Cari nomor surat, penerima atau isi instruksi..." autocomplete="off" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-xs text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 sm:text-sm">
                    </div>
                </div>
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        <?php if($hasFilters): ?>
                            <a href="<?php echo e(route('disposisi.index')); ?>" title="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </a>
                        <?php endif; ?>
                    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    <?php $__empty_1 = true; $__currentLoopData = $disposisis ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="transition duration-150 hover:bg-slate-50/60">
                            <td class="px-4 py-3.5 text-slate-700 sm:px-5 sm:py-4"><span class="inline-block max-w-[190px] truncate font-medium" title="<?php echo e($nomorSurat); ?>"><?php echo e($nomorSurat); ?></span></td>
                            <td class="px-4 py-3.5 text-slate-500 sm:px-5 sm:py-4"><?php echo e($tanggalDisposisi); ?></td>
                            <td class="px-4 py-3.5 sm:px-5 sm:py-4">
                                <?php if($penerimaData['nama'] !== '-'): ?>
                                    <div class="flex min-w-[150px] max-w-[200px] flex-col rounded-lg border border-slate-200/60 bg-slate-100 px-2.5 py-1.5" title="<?php echo e($penerimaData['nama']); ?><?php echo e($penerimaData['jabatan'] ? ' - '.$penerimaData['jabatan'] : ''); ?>">
                                        <span class="truncate font-semibold text-slate-700"><?php echo e($penerimaData['nama']); ?></span>
                                        <?php if($penerimaData['jabatan']): ?>
                                            <span class="truncate text-[10px] text-slate-400"><?php echo e($penerimaData['jabatan']); ?></span>
                                        <?php endif; ?>
                                    </div>
                                <?php else: ?>
                            ...
                            <td class="max-w-[260px] truncate px-4 py-3.5 font-medium text-slate-700 sm:px-5 sm:py-4" title="<?php echo e($instruksi); ?>"><?php echo e($instruksi); ?></td>
                            <td class="px-4 py-3.5 sm:px-5 sm:py-4"><span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold <?php echo e($badgeClass); ?>"><?php echo e($statusOptions[$status] ?? ucfirst($status)); ?></span></td>
                            <td class="px-4 py-3.5 text-slate-500 sm:px-5 sm:py-4">
                                <?php if($batasWaktu !== '-'): ?>
                                    <span class="<?php echo e($isLate ? 'font-semibold text-rose-600' : ''); ?>"><?php echo e($batasWaktu); ?></span>
                                <?php else: ?>
                                    <span class="text-slate-400">-</span>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3.5 text-center sm:px-5 sm:py-4">
                                <div class="inline-flex items-center gap-1">
                                    <?php if(Route::has('disposisi.show')): ?>
                                        <a href="<?php echo e(route('disposisi.show', $d)); ?>" title="Lihat Detail" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-blue-50 hover:text-blue-600">
                                    ...
                                    <?php endif; ?>
                                    <?php if(in_array($userRole, ['admin','pimpinan'], true) && Route::has('disposisi.edit')): ?>
                                        <a href="<?php echo e(route('disposisi.edit', $d)); ?>" title="Ubah Disposisi" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-amber-50 hover:text-amber-600">
                                    ...
                                    <?php endif; ?>
                                    <?php if(in_array($userRole, ['admin','pimpinan'], true) && Route::has('disposisi.destroy')): ?>
                                        <form action="<?php echo e(route('disposisi.destroy', $d)); ?>" method="POST" class="delete-form inline">
                                            <?php echo csrf_field(); ?>
        ...
        <?php if(isset($disposisis) && method_exists($disposisis, 'hasPages') && $disposisis->hasPages()): ?>
            <div class="border-t border-slate-100 px-4 py-3 sm:px-6 sm:py-4"><?php echo e($disposisis->withQueryString()->links()); ?></div>
        <?php endif; ?>
    </div>
</div>
