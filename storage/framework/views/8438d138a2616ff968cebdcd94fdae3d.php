...
<style>
.date-range-input{cursor:pointer}.date-range-input::selection{background:transparent}.daterangepicker{z-index:99999!important;width:auto;border:1px solid #e2e8f0;border-radius:18px;background:#fff;box-shadow:0 20px 50px rgba(15,23,42,.14),0 8px 20px rgba(15,23,42,.06);font-family:inherit;padding:10px}.daterangepicker .calendar-table{border:0;background:transparent}.daterangepicker .calendar-table table{border-collapse:separate;border-spacing:2px}.daterangepicker .calendar-table th{color:#94a3b8;font-size:10px;font-weight:700;text-transform:uppercase}.daterangepicker .calendar-table td{width:34px;height:34px;border-radius:10px;color:#475569;font-size:11px;font-weight:500;transition:background-color .15s ease,color .15s ease}.daterangepicker td.available:hover,.daterangepicker th.available:hover{background:#eff6ff;color:#2563eb}.daterangepicker td.in-range{background:#eff6ff;color:#2563eb}.daterangepicker td.active,.daterangepicker td.active:hover{background:#2563eb;color:#fff;border-radius:999px}.daterangepicker td.start-date{border-radius:999px 0 0 999px}.daterangepicker td.end-date{border-radius:0 999px 999px 0}.daterangepicker td.start-date.end-date{border-radius:999px}.daterangepicker td.off,.daterangepicker td.off.in-range{background:transparent;color:#cbd5e1}.daterangepicker .prev,.daterangepicker .next{border-radius:8px;color:#64748b;transition:background-color .15s ease,color .15s ease}.daterangepicker .prev:hover,.daterangepicker .next:hover{background:#f1f5f9;color:#2563eb}.daterangepicker select.monthselect,.daterangepicker select.yearselect{width:auto;border:0;border-radius:8px;background:#f8fafc;color:#334155;font-size:12px;font-weight:700;padding:5px 8px;outline:none;cursor:pointer}.daterangepicker select.monthselect:focus,.daterangepicker select.yearselect:focus{background:#eff6ff;color:#2563eb}.daterangepicker .drp-buttons{border-top:1px solid #e2e8f0;margin-top:8px;padding:10px 5px 2px}.daterangepicker .drp-selected{color:#64748b;font-size:11px;font-weight:600}.daterangepicker .applyBtn{border:0;border-radius:10px;background:#2563eb;color:#fff;font-size:11px;font-weight:700;padding:8px 14px;transition:background-color .15s ease}.daterangepicker .applyBtn:hover{background:#1d4ed8}.daterangepicker .cancelBtn{border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc;color:#475569;font-size:11px;font-weight:700;padding:8px 14px;transition:background-color .15s ease,color .15s ease}.daterangepicker .cancelBtn:hover{background:#f1f5f9;color:#334155}
@media(max-width:767px){.daterangepicker{position:fixed!important;top:50%!important;left:50%!important;right:auto!important;bottom:auto!important;width:calc(100vw - 20px)!important;max-width:420px;max-height:calc(100vh - 30px);overflow-y:auto;transform:translate(-50%,-50%)}.daterangepicker .calendar{float:none!important;width:100%!important;max-width:100%!important;margin:0!important}.daterangepicker .calendar.right{margin-top:10px!important}.daterangepicker .calendar-table td{width:36px;height:36px}body.daterangepicker-open{overflow:hidden}}
</style>
<?php $__env->stopPush(); ?>
...
<div class="space-y-4 sm:space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="grid w-full grid-cols-3 gap-2 sm:flex sm:w-auto">
            <a href="<?php echo e(route('export.surat-masuk.excel', request()->query())); ?>" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs font-semibold text-emerald-700 shadow-sm transition hover:bg-emerald-100 sm:px-3.5">
            ...
            <a href="<?php echo e(route('export.surat-masuk.pdf', request()->query())); ?>" target="_blank" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-2 text-xs font-semibold text-rose-700 shadow-sm transition hover:bg-rose-100 sm:px-3.5">
            ...
            <?php if(in_array($userRole, ['admin', 'pimpinan'])): ?>
                <a href="<?php echo e(route('surat-masuk.create')); ?>" class="inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-2.5 py-2 text-xs font-semibold text-white shadow-md shadow-blue-600/20 transition hover:bg-blue-700 sm:px-4">
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-3 shadow-sm sm:p-5">
        <form method="GET" action="<?php echo e(route('surat-masuk.index')); ?>" id="filterForm" class="space-y-3">
            <div class="grid grid-cols-1 gap-2.5 lg:grid-cols-12">
                <div class="lg:col-span-5">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="search" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari perihal atau nomor surat..." autocomplete="off" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-xs text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20 sm:text-sm">
                    </div>
                </div>
                ...
                <div class="lg:col-span-3">
                    <div class="flex h-11 gap-2">
                        ...
                        <?php if($hasFilters): ?>
                            <a href="<?php echo e(route('surat-masuk.index')); ?>" title="Reset Filter" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </a>
                        <?php endif; ?>
                    </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
            ...
            <table class="w-full min-w-[850px] border-collapse whitespace-nowrap text-left text-xs sm:text-sm">
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    <?php $__empty_1 = true; $__currentLoopData = $suratMasuks ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php
                            $status = strtolower($s->status ?? 'baru');
                        ...
                        <tr class="transition duration-150 hover:bg-slate-50/60">
                            <td class="px-4 py-3.5 text-slate-500 sm:px-6 sm:py-4"><?php echo e(optional($s->tanggal_terima)->format('d/m/Y') ?? '-'); ?></td>
                            <td class="px-4 py-3.5 text-slate-700 sm:px-6 sm:py-4">
                                <span class="inline-block max-w-[220px] truncate rounded-lg border border-slate-200/60 bg-slate-100 px-2.5 py-1 font-medium text-slate-700"><?php echo e($s->pengirim ?? '-'); ?></span>
                            </td>
                            ...
                            <td class="px-4 py-3.5 sm:px-6 sm:py-4">
                                <span class="inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold <?php echo e($badgeClass); ?>"><?php echo e(ucfirst($status)); ?></span>
                            </td>
                            <td class="px-4 py-3.5 text-center sm:px-6 sm:py-4">
                                <div class="inline-flex items-center gap-1">
                                    <a href="<?php echo e(route('surat-masuk.show', $s)); ?>" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-blue-50 hover:text-blue-600" title="Lihat Detail & Disposisi">
                                    ...
                                    <?php if(in_array($userRole, ['admin', 'pimpinan'])): ?>
                                        <a href="<?php echo e(route('surat-masuk.edit', $s)); ?>" class="rounded-lg p-1.5 text-slate-400 transition hover:bg-amber-50 hover:text-amber-600" title="Ubah Data">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </a>
                                        <form action="<?php echo e(route('surat-masuk.destroy', $s)); ?>" method="POST" class="delete-form inline">
                                            <?php echo csrf_field(); ?>
        ...
        <?php if(isset($suratMasuks) && method_exists($suratMasuks, 'hasPages') && $suratMasuks->hasPages()): ?>
            <div class="border-t border-slate-100 px-4 py-3 sm:px-6 sm:py-4"><?php echo e($suratMasuks->withQueryString()->links()); ?></div>
        <?php endif; ?>
    </div>
</div>
...
(function(){
    ...
    async function initializeSuratMasukPage(){
        try{
    ...
    function initializePage(){
        ...
        if(typeof Swal!=='undefined'){
            ...
            document.querySelectorAll('.delete-btn').forEach(function(button){
                button.addEventListener('click',function(){
                    const form=this.closest('.delete-form');
                    if(form&&confirm('Yakin ingin menghapus surat masuk ini?')) form.submit();
                });
            });
        }
    ...
    if(document.readyState==='loading'){
        document.addEventListener('DOMContentLoaded',initializeSuratMasukPage);
    }else{
        initializeSuratMasukPage();
    }
})();
</script>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\E-Arsip\resources\views/surat_masuk/index.blade.php ENDPATH**/ ?>